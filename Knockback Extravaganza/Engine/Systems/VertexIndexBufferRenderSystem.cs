    ...
    public class VertexIndexBufferRenderSystem<T> : IRenderSystem where T : struct, IVertexType {
        public void Render(GameTime gameTime, GraphicsDeviceManager graphicsDevice, ComponentManager componentManager) {
            ...
            foreach (KeyValuePair<Entity, IComponent> component in components) {
                ...
                CollisionComponent collC = componentManager.GetComponent<CollisionComponent>(component.Key);
                if (transform != default(TransformComponent) /*&& InView(camera, collC)*/) {
        ...
        //public bool InView(CameraComponent camera, CollisionComponent collC)
        //{
        //    return new BoundingFrustum(camera.View * camera.Projection).Intersects(collC.BoundingSphere);
        //}
    }
}
